...
class Simulation:
    ...
    def run(self):
        ...
        global_vars.init(Value, ctypes)
        global_vars.world.value = self.world

        with timed_performance("Actors trained"):

            # with Pool(processes=self.population.actor_count) as pool:
            with Pool(processes=len(os.sched_getaffinity(0))) as pool:
                for g in range(max_generation):
                    print(f"Starting generation {g}")
                    self.population.actors = pool.map(func=self.process_actor, iterable=self.population.actors)
    ...
    def process_actor(self, actor):
        rows = min(len(global_vars.world.value.dataframe.index), 5000)

        print('SUB PROCESS')

        for timestamp, t in global_vars.world.value.dataframe.head(rows).iterrows():
            actor.run(t)
