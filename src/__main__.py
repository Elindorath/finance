...
@inject
def main(simulation: Simulation = Provide[Container.simulation_factory]) -> None:
    ...
    with timed_performance(prefix='Simulation ran'):
...
def confirm(*, prompt: str = 'Confirm', default_answer: bool = True) -> bool:
...
def existing_file_path(file_path) -> str:
    from pathlib import Path
...
def warn_existing_file_path(file_path) -> str:
    from pathlib import Path
    ...
    if absolute_path.exists():
        if not absolute_path.is_file():
            error_message = f'The file {file_path} already exists and is not a file (resolved to {absolute_path_str})'
        else:
            logging.warning(f'The file {file_path} already exists, it will be overwritten (resolved to {absolute_path_str})')

            if not confirm(prompt='Are you sure to overwrite it?'):
                error_message = f'You have not given permission to overwrite {file_path}'
...
def get_config_from_args(_args):
    _config = {}

    for key, value in _args.items():
        if key in ['genomes', 'save_genomes']:
            _config[key] = Path.cwd().joinpath(value).resolve()
        elif key == 'markets':
            _config[key] = [
                Path.cwd().joinpath(market_path).resolve()
                for market_path in value
            ]
        else:
            _config[key] = value
